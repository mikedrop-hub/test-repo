...
def main():
    ...
    if not os.path.exists(DATEI):
        with open(DATEI, 'w') as f:
            pass 
    if hello.get_row_by_index(BALANCE_FILE, 0) == EMPTY_ROW: 
        with open(BALANCE_FILE, 'a') as f:
            writer = csv.writer(f)
            writer.writerow(["Total balance", "0.0"]) # This hopefully ensures that there is always a number in the first row of the balance file
